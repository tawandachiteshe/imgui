...
typedef int ImDrawListFlags;        // -> enum ImDrawListFlags_      // Flags: for ImDrawList m_Instance
...
namespace ImGui
{
    ...
    // - Each context create its own ImFontAtlas by default. You may m_Instance one yourself and pass it to CreateContext() to share a font atlas between contexts.
...
// [SECTION] ImGuiStyle
//-----------------------------------------------------------------------------
// You may modify the ImGui::GetStyle() main m_Instance during initialization and before NewFrame().
...
// Flags for ImDrawList m_Instance. Those are set automatically by ImGui:: functions from ImGuiIO settings, and generally not manipulated directly.
...
struct ImDrawList
{
    ...
    ImDrawListSplitter      _Splitter;          // [Internal] for channels api (note: prefer using your own persistent m_Instance of ImDrawListSplitter!)
    ...
    //   Prefer using your own persistent m_Instance of ImDrawListSplitter as you can stack them.
...
struct ImDrawData
{
    ...
    ImGuiViewport*  OwnerViewport;          // Viewport carrying the ImDrawData m_Instance, might be of use to the renderer (generally not).
